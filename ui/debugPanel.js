...
export var dbgDefs = [
  ...
  { stem:'midground', get:function(){ return SCENE_ART.MIDGROUND_VISIBLE ? 1 : 0; }, set:function(v){ SCENE_ART.MIDGROUND_VISIBLE = Math.round(v) > 0; }, step:1, fmt:function(v){ return Math.round(v) > 0 ? 'ON' : 'OFF'; } },
  ...
  { stem:'cloudop', get:function(){ return SCENE_ART.CLOUD_OPACITY; }, set:function(v){ SCENE_ART.CLOUD_OPACITY = Math.max(0, Math.min(0.6, v)); }, step:0.02, fmt:function(v){ return v.toFixed(2); } },
  { stem:'midop', get:function(){ return SCENE_ART.MIDGROUND_OPACITY; }, set:function(v){ SCENE_ART.MIDGROUND_OPACITY = Math.max(0, Math.min(0.7, v)); }, step:0.02, fmt:function(v){ return v.toFixed(2); } },
  { stem:'flowerop', get:function(){ return SCENE_ART.FLOWER_OPACITY; }, set:function(v){ SCENE_ART.FLOWER_OPACITY = Math.max(0, Math.min(0.7, v)); }, step:0.02, fmt:function(v){ return v.toFixed(2); } },
  { stem:'foreop', get:function(){ return SCENE_ART.FOREGROUND_OPACITY; }, set:function(v){ SCENE_ART.FOREGROUND_OPACITY = Math.max(0, Math.min(0.6, v)); }, step:0.02, fmt:function(v){ return v.toFixed(2); } },
  { stem:'wind', get:function(){ return SCENE_ART.WIND_SWAY_INTENSITY; }, set:function(v){ SCENE_ART.WIND_SWAY_INTENSITY = Math.max(0, Math.min(2.0, v)); }, step:0.05, fmt:function(v){ return v.toFixed(2); } },
  { stem:'clouddrift', get:function(){ return SCENE_ART.CLOUD_DRIFT_STRENGTH; }, set:function(v){ SCENE_ART.CLOUD_DRIFT_STRENGTH = Math.max(0, Math.min(2.0, v)); }, step:0.05, fmt:function(v){ return v.toFixed(2); } },
  { stem:'swarm', get:function(){ return SCENE_ART.AMBIENT_SWARM_INTENSITY; }, set:function(v){ SCENE_ART.AMBIENT_SWARM_INTENSITY = Math.max(0, Math.min(2.0, v)); }, step:0.05, fmt:function(v){ return v.toFixed(2); } },
  { stem:'foresway', get:function(){ return SCENE_ART.FOREGROUND_SWAY_INTENSITY; }, set:function(v){ SCENE_ART.FOREGROUND_SWAY_INTENSITY = Math.max(0, Math.min(2.0, v)); }, step:0.05, fmt:function(v){ return v.toFixed(2); } },
  ...
  { stem:'sunx', get:function(){ return LIGHTING.KEY_POS_X; }, set:function(v){ LIGHTING.KEY_POS_X = Math.max(-20, Math.min(20, v)); refreshLightingRigRef(); }, step:0.5, fmt:function(v){ return v.toFixed(1); } },
  { stem:'suny', get:function(){ return LIGHTING.KEY_POS_Y; }, set:function(v){ LIGHTING.KEY_POS_Y = Math.max(4, Math.min(30, v)); refreshLightingRigRef(); }, step:0.5, fmt:function(v){ return v.toFixed(1); } },
  { stem:'sunz', get:function(){ return LIGHTING.KEY_POS_Z; }, set:function(v){ LIGHTING.KEY_POS_Z = Math.max(-20, Math.min(20, v)); refreshLightingRigRef(); }, step:0.5, fmt:function(v){ return v.toFixed(1); } },
  { stem:'rimx', get:function(){ return LIGHTING.RIM_POS_X; }, set:function(v){ LIGHTING.RIM_POS_X = Math.max(-20, Math.min(20, v)); refreshLightingRigRef(); }, step:0.5, fmt:function(v){ return v.toFixed(1); } },
  { stem:'rimy', get:function(){ return LIGHTING.RIM_POS_Y; }, set:function(v){ LIGHTING.RIM_POS_Y = Math.max(2, Math.min(24, v)); refreshLightingRigRef(); }, step:0.5, fmt:function(v){ return v.toFixed(1); } },
  { stem:'rimz', get:function(){ return LIGHTING.RIM_POS_Z; }, set:function(v){ LIGHTING.RIM_POS_Z = Math.max(-20, Math.min(20, v)); refreshLightingRigRef(); }, step:0.5, fmt:function(v){ return v.toFixed(1); } },
  { stem:'hiveglow', get:function(){ return LIGHTING.HIVE_GLOW_INTENSITY; }, set:function(v){ LIGHTING.HIVE_GLOW_INTENSITY = Math.max(0, Math.min(4, v)); refreshLightingRigRef(); }, step:0.1, fmt:function(v){ return v.toFixed(1); } },
  ...
  { stem:'readyfx', get:function(){ return PRESENTATION.HONEY_READY_FX_INTENSITY; }, set:function(v){ PRESENTATION.HONEY_READY_FX_INTENSITY = Math.max(0, Math.min(2.0, v)); }, step:0.05, fmt:function(v){ return v.toFixed(2); } },
  { stem:'dripcount', get:function(){ return PRESENTATION.HONEY_READY_DRIP_COUNT; }, set:function(v){ PRESENTATION.HONEY_READY_DRIP_COUNT = Math.max(0, Math.min(3, Math.round(v))); }, step:1, fmt:function(v){ return Math.round(v) + ''; } },
  { stem:'overflowfx', get:function(){ return PRESENTATION.HONEY_OVERFLOW_INTENSITY; }, set:function(v){ PRESENTATION.HONEY_OVERFLOW_INTENSITY = Math.max(0, Math.min(2.0, v)); }, step:0.05, fmt:function(v){ return v.toFixed(2); } },
  { stem:'overflowct', get:function(){ return PRESENTATION.HONEY_OVERFLOW_COUNT; }, set:function(v){ PRESENTATION.HONEY_OVERFLOW_COUNT = Math.max(4, Math.min(20, Math.round(v))); }, step:1, fmt:function(v){ return Math.round(v) + ''; } },
  { stem:'combofx', get:function(){ return PRESENTATION.COMBO_FEEDBACK_INTENSITY; }, set:function(v){ PRESENTATION.COMBO_FEEDBACK_INTENSITY = Math.max(0, Math.min(2.0, v)); }, step:0.05, fmt:function(v){ return v.toFixed(2); } },
  { stem:'screenbloom', get:function(){ return PRESENTATION.SCREEN_REWARD_BLOOM; }, set:function(v){ PRESENTATION.SCREEN_REWARD_BLOOM = Math.max(0, Math.min(2.0, v)); }, step:0.05, fmt:function(v){ return v.toFixed(2); } },
  { stem:'bloomdur', get:function(){ return PRESENTATION.SCREEN_BLOOM_DURATION; }, set:function(v){ PRESENTATION.SCREEN_BLOOM_DURATION = Math.max(0.12, Math.min(1.1, v)); }, step:0.02, fmt:function(v){ return v.toFixed(2) + 's'; } },
  { stem:'screenshake', get:function(){ return PRESENTATION.SCREEN_SHAKE_INTENSITY; }, set:function(v){ PRESENTATION.SCREEN_SHAKE_INTENSITY = Math.max(0, Math.min(2.0, v)); }, step:0.05, fmt:function(v){ return v.toFixed(2); } },
  { stem:'shakedur', get:function(){ return PRESENTATION.SCREEN_SHAKE_DURATION; }, set:function(v){ PRESENTATION.SCREEN_SHAKE_DURATION = Math.max(0.10, Math.min(0.80, v)); }, step:0.02, fmt:function(v){ return v.toFixed(2) + 's'; } },
  { stem:'mergefuse', get:function(){ return PRESENTATION.MERGE_FUSE_INTENSITY; }, set:function(v){ PRESENTATION.MERGE_FUSE_INTENSITY = Math.max(0, Math.min(2.0, v)); }, step:0.05, fmt:function(v){ return v.toFixed(2); } },
  { stem:'unlocklight', get:function(){ return PRESENTATION.UNLOCK_LIGHT_INTENSITY; }, set:function(v){ PRESENTATION.UNLOCK_LIGHT_INTENSITY = Math.max(0, Math.min(2.0, v)); }, step:0.05, fmt:function(v){ return v.toFixed(2); } },
  { stem:'unlockdur', get:function(){ return PRESENTATION.UNLOCK_LIGHT_DURATION; }, set:function(v){ PRESENTATION.UNLOCK_LIGHT_DURATION = Math.max(0.25, Math.min(2.0, v)); }, step:0.05, fmt:function(v){ return v.toFixed(2) + 's'; } },
  { stem:'beearc', get:function(){ return PRESENTATION.BEE_ARC_HEIGHT; }, set:function(v){ PRESENTATION.BEE_ARC_HEIGHT = Math.max(0.2, Math.min(2.0, v)); }, step:0.05, fmt:function(v){ return v.toFixed(2); } },
  { stem:'beesway', get:function(){ return PRESENTATION.BEE_ARC_SIDE_SWAY; }, set:function(v){ PRESENTATION.BEE_ARC_SIDE_SWAY = Math.max(0, Math.min(2.0, v)); }, step:0.05, fmt:function(v){ return v.toFixed(2); } },
  { stem:'beeface', get:function(){ return PRESENTATION.BEE_FACING_BLEND; }, set:function(v){ PRESENTATION.BEE_FACING_BLEND = Math.max(0.2, Math.min(2.0, v)); }, step:0.05, fmt:function(v){ return v.toFixed(2); } },
  { stem:'beehover', get:function(){ return PRESENTATION.BEE_IDLE_HOVER; }, set:function(v){ PRESENTATION.BEE_IDLE_HOVER = Math.max(0, Math.min(2.0, v)); }, step:0.05, fmt:function(v){ return v.toFixed(2); } },
  { stem:'beeattach', get:function(){ return PRESENTATION.BEE_WORK_ATTACH; }, set:function(v){ PRESENTATION.BEE_WORK_ATTACH = Math.max(0.2, Math.min(1.8, v)); }, step:0.05, fmt:function(v){ return v.toFixed(2); } },
  { stem:'trailden', get:function(){ return PRESENTATION.BEE_TRAIL_DENSITY; }, set:function(v){ PRESENTATION.BEE_TRAIL_DENSITY = Math.max(0, Math.min(1.5, v)); }, step:0.05, fmt:function(v){ return v.toFixed(2); } },
  { stem:'trailop', get:function(){ return PRESENTATION.BEE_TRAIL_OPACITY; }, set:function(v){ PRESENTATION.BEE_TRAIL_OPACITY = Math.max(0, Math.min(1.6, v)); }, step:0.05, fmt:function(v){ return v.toFixed(2); } },
